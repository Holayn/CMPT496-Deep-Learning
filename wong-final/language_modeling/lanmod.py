...
num_steps = int(sys.argv[1]) # 20
#The number of processing units (neurons) in the hidden layers
hidden_size = int(sys.argv[2]) # 200
...
##########################################################################################################################
def get_word(session, m, primer, words_len, eval_op, word_to_id, verbose=False):
    words_out = [primer]
    ...
    for n in range(words_len):
        ...
        batch_len = data_len // batch_size
        data = np.zeros([batch_size, batch_len], dtype=np.int32)
        data[batch_size-1][-raw_data.size:] = raw_data
...
with tf.Graph().as_default(), tf.Session() as session:
    ...
    for i in range(max_max_epoch):
        ...
        # TEMP words = get_word(session, mtest, "The", 19, tf.no_op(), word_to_id, verbose=False)
        # TEMP print('Sample sentence: ' + ' '.join(words))
